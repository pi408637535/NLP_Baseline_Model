# -*- coding: utf-8 -*-
from com.study.pytorchtools import EarlyStopping
import torch
...
from com.study import config as argumentparser
...
def get_test_result(model,criterion,data_iter,data_set):
    # 生成测试结果
    model.eval()
    data_loss = 0
    true_sample_num = 0
...
def train(acc = 0):
    for i in range(0, 10):  # 10-cv
        ...
        for epoch in range(config.epoch):
            ...
            # 一轮训练结束，在验证集测试
            valid_loss, valid_acc = get_test_result(model,criterion,valid_iter, valid_set)
        ...
        test_loss, test_acc = get_test_result(model,criterion,test_iter, test_set)
